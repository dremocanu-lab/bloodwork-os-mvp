...
def normalize_reference_range(value: Any) -> str | None:
    ...
    # Most Romanian CBC reference ranges are non-negative; a leading minus on the
    # second value is usually the separator attached by OCR.
...
def normalize_unit(value: Any) -> str | None:
    ...
    if lowered in {"ui/ml", "uiu/ml", "uiu/ml", "uiu/ml"}:
        return "uIU/mL"
...
def standardize_lab_row_shape(lab: dict[str, Any]) -> dict[str, Any]:
    ...
    # Safety rule: never call something Normal unless a reference range exists.
...
def extract_google_labs(extraction: dict[str, Any], extracted_text: str, warnings: list[str]) -> list[dict[str, Any]]:
...
def process_bloodwork_document(extraction: dict[str, Any], extracted_text: str, warnings: list[str]) -> dict[str, Any]:
