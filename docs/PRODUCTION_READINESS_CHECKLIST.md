...
| 52 | AI governance / Ask Bragi | `[IMPLEMENTED — ACTIVE]` — every forward-looking §2 item in `docs/ai/AI_GOVERNANCE.md` is now a tested control; `NEXT_PUBLIC_ASK_BRAGI_ENABLED=true` in Vercel production and `ASK_BRAGI_ENABLED=true` + a real `OPENAI_API_KEY` on Render — confirmed live via a real synthetic-account smoke test and real streaming/stop/conversation-history/reverse-language verification against production. Remaining: imaging + medication-conflict evals, a proper latency benchmark — `BRAGI_ASK_BRAGI_PLAN.md` |
